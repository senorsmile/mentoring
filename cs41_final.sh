...
show_banner(){
  ...
  echo -e "${MENU}*********************************************${NORMAL}"
  echo -e " ";
  echo -e "${MENU}${NUMBER}Name of Server:${MENU}\n$(hostname -s)${NORMAL}"
  echo -e "${MENU}    ****-****-****-****  ${NORMAL}"
  echo -e "${MENU}${NUMBER}Server IP Address:${MENU}\n$(hostname -i)${NORMAL}"
  echo -e "${MENU}    ****-****-****-****  ${NORMAL}"
  echo -e "${MENU}${NUMBER}Number of Current Users:${MENU}\n$(who)${NORMAL}"
  echo -e "${MENU}    ****-****-****-****  ${NORMAL}"
  echo -e "${MENU}${NUMBER}Date and Time:${MENU}\n$(date)${NORMAL}"
  echo -e "${MENU}    ****-****-****-****  ${NORMAL}"
  echo -e "${MENU}${NUMBER}OS Version:${MENU}\n$(lsb_release -a)${NORMAL}"
  echo -e "${MENU}    ****-****-****-****  ${NORMAL}"
  echo -e "${MENU}${NUMBER}Current User Name:${MENU}\n$(whoami)${NORMAL}"
  echo -e " ";
  echo -e "${MENU}*********************************************${NORMAL}"
  echo -e "${MENU}*********************************************${NORMAL}"
}
...
#----- Option 2 -----
listActiveProcess(){
  echo "List of active processes for user: $userName"; 
  ps -u $userName
}

#----- Option 3 -----
monitorActiveProcess(){
  echo "Monitoring of active processes for user: $userName";
  echo -e "${RED_TEXT}*****ctrl-c to return to menu*****";
  top -u $userName
}
...
while [ userChoice != '0' ]
    ...
    if [[ $userChoice = "0" ]]; then
      ...
      case $userChoice in
        ...
        2) clear;
        option_picked "Option 2: List of active processes for user";
        echo " ";
        currentUsers;
        echo " ";
        echo "Please select from list of current users:";
        read userName;
        echo " "; 
        listActiveProcess;
        show_menu;       
        ...
        3) clear;
        option_picked "Option 3: Monitor active processes for user";
        echo " ";
        currentUsers;
        echo " ";
        echo "Please select from list of current users:";
